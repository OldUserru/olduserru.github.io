import { auth, db, signInWithEmailAndPassword, createUserWithEmailAndPassword, onAuthStateChanged, GoogleAuthProvider, signInWithRedirect, getRedirectResult, ref, set, get, update, onValue } from "./firebase-config.js";
...
getRedirectResult(auth).then(async (result) => {
    if (result && result.user) {
        ...
        const snap = await get(ref(db, "terminals/" + uid));
        if (!snap.exists()) {
            await set(ref(db, "terminals/" + uid), {
    ...
    if(authError) authError.textContent = "Ошибка входа Google: " + error.message;
});
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        // Listen to real-time updates from RTDB
        onValue(ref(db, "terminals/" + currentUserUid), (snap) => {
            if (snap.exists()) {
                ...
                set(ref(db, "terminals/" + currentUserUid), {
                    state: 'idle',
                    amount: "0",
                    balance: 0,
                    history: [],
...
if (btnGoogleLogin) {
    btnGoogleLogin.addEventListener('click', () => {
        authError.textContent = "Перенаправление на Google...";
